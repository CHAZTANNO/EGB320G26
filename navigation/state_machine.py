    ...
    def run(self, navSys):
        ...
        if navSys.dataDict['rowMarkerRB'][rowNo] != None:
            ...
            if navSys.dataDict['wallPoints'] != None:
                if navSys.dataDict['wallPoints'][0] != None:
                    if navSys.dataDict['wallPoints'][0][1] != None:
                        if navSys.dataDict['wallPoints'][0][1] <= 0.1:
                            event = 'arrived_at_bay'
            else:
                event = 'lost_in_row'
        else:
                    ...
                    for item in items:
                        if item != None:
                            if isinstance(item, float):
                                if (items[1] <= 0.25 and items[1] >= -0.25) and items[0] < 0.25:
                                    event = 'facing_bay'
                            elif item[0] < 0.25:
