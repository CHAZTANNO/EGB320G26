...
class visionSystem:
    ...
    def processTasks(self, frame, blackThreshold, greenThreshold, blueThreshold, squareThreshold, yellowThreshold): #maybe others?
        print("im in processing tasks now")
        tasks = [(self.rowMarker, (frame, blackThreshold)),
            (self.obstacle, (frame, greenThreshold)),
            (self.shelves, (frame, blueThreshold)),
            (self.packingBay, (frame, squareThreshold, yellowThreshold))
        ]

        results = self.pool.map(lambda task: task[0](*task[1]), tasks)
    ...
    def processImage(self):
        while(1):
            ...
            returnVals = self.processTasks(frameHSV, self.blackThreshold, self.greenThreshold, self.blueThreshold, self.squareThreshold, self.yellowThreshold)
