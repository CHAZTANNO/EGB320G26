...
class visionSystem:
    ...
    def rowMarker(self, frameHSV, thresholdVals):
        ...
        if cv2.countNonZero(mask) > self.minArea:
            ...
            #     yText = minY - 10
            # xText,yText
                cv2.putText(frameGlobal, f"R{circleCount}, {meanRange:.0f}mm, {meanBearing:.1f}*", (minX,minY-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 1)
    ...
    def obstacle(self, frame, thresholdVals):
        ...
        if cv2.countNonZero(mask) > self.minArea:
            ...
            for contour in contours:
                ...
                # else:
                #     yText = y - 10
                cv2.putText(frameGlobal, f"Obstacle, {range:.0f}mm, {bearing:.1f}*", (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,255), 1)
            
            cv2.drawContours(frameGlobal, contours, -1, (255,0,255), 2)
            return outputRB
    ...
    def shelves(self, frame, thresholdVals):
        ...
        if cv2.countNonZero(mask) > self.minArea:
            ...
            for contour in contours:
                ...
                # else:
                #     yText = y - 10
                cv2.putText(frameGlobal, f"Shelf, {range:.0f}mm, {bearing:.1f}*", (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 1)
            
            cv2.drawContours(frameGlobal, contours, -1, (0,255,0), 3)
    ...
    def packingBay(self, frame, threshValsSquare, threshValsYel):
        ...
        if len(usefulContour) > 0: #usefulContour has a value - something was identified
            cv2.drawContours(frameGlobal, [usefulContour], 0, (173, 13, 106), 2)
            cv2.putText(frameGlobal, f"PBay, {range:.0f}mm, {bearing:.1f}*", (usefulX,usefulY-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (173, 13, 106), 1)
...
while(1):
    ...
    frameGlobal = vision_system.currentFrame
    ...
    cv2.putText(frameGlobal, f"FPS:{fps:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    print("im:", str(timeforImage), "R:", str(timeforRow), "O:", str(timeforObs), "S:", str(timeforShel), "PB:", str(timeforPb))
    
    frameLarge = cv2.resize(frameGlobal, (1200, 900), interpolation=cv2.INTER_LINEAR)
