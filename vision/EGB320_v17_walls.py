...
class visionSystem:
    def __init__(self):
        ...
        self.wallThreshold = [20, 65, 15, 70, 163, 255]
    ...
    def walls(self, writeFrame, frame, thresholdVals):
        ...
        if pointCount == 1:
            ...
            cv2.circle(writeFrame, (midx,midy), 10, self.colWall, -1)
            ...
            cv2.circle(writeFrame, (x1,y1), 10, self.colWall, -1)
            cv2.circle(writeFrame, (x2,y2), 10, self.colWall, -1)
    ...
    def GetDetectedWallPoints(self):
        ...
        globalFrame, outputRB = self.walls(globalFrame, frameHSV, self.wallThreshold)

        frameLarge = cv2.resize(globalFrame, (600, 450), interpolation=cv2.INTER_LINEAR)
