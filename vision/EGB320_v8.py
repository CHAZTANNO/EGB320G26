...
class visionSystem:
    def __init__(self):
        ...
        self.frameSizeX = 400#820
        self.frameSizeY = 300#616
        self.minArea = self.frameSizeX*self.frameSizeY / 100
        self.currentFrame = np.zeros((self.frameSizeX, self.frameSizeY, 3), np.uint8)
        scale = 0.6
        # resolutionX = 480#int(scale*self.frameSizeX)
        # resolutionY = 270#int(scale*frameSizeY)
        frameCount = 0
        ...
        self.yellowThreshold = [21, 35, 205, 255, 195, 255]
    ...
    def rowMarker(self, frameHSV, thresholdVals):
        ...
        if (0 < circleCount <= 3):
            ...
            minY = np.min(ys)
        # else:
        #     return (None, None, None) #Not sure if good idea - revise later
    ...
    def obstacle(self, frame, thresholdVals):
        ...
        for contour in contours:
            ...
            # else:
            #     yText = y - 10
            cv2.putText(frame, f"Obstacle, {range:.0f}mm, {bearing:.1f}*", (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,255), 1)
    ...
    def shelves(self, frame, thresholdVals):
        ...
        for contour in contours:
            ...
            # else:
            #     yText = y - 10
            cv2.putText(frame, f"Shelf, {range:.0f}mm, {bearing:.1f}*", (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 1)
    ...
    def packingBay(self, frame, threshValsSquare, threshValsYel):
        ...
        contours = self.contourImage(frame, maskSquare)
        squareCount = 0
        for contour in contours:
            squareStatus, x, w = self.isSquare(contour)
        ...
        if squareCount != 1: #either no squares, or multiple squares - use yellow instead
            ...
            contours = self.contourImage(frame, maskYel)

            if contours:  # Make sure there are contours
                largestContour = max(contours, key=cv2.contourArea) #only one packing station, so just take the largest
                x,y,w,h = cv2.boundingRect(largestContour)
                range = self.findRangeHeight(60, h)
                bearing = self.findBearing(x, w)
    ...
    def isSquare(self, contour):
        ...
        if len(approx) == 4:
            ...
            if 0.95 <= aspectRatio <= 1.05:  # Check if aspect ratio is close to 1 (square)
...
while(1):
    ...
    frame = vision_system.currentFrame
    ...
    cv2.putText(frame, f"FPS:{fps:.2f} im:{timeforImage:.2f} r:{timeforRow:.2f} o:{timeforObs:.2f} s:{timeforShel:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
