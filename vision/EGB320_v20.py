...
class visionSystem:
    def __init__(self):
        ...
        self.cap.set_controls({"ExposureTime": 200000, "AnalogueGain": 2.0, "ColourGains": (1.4,1.5)})
        ...
        self.blueThreshold = [93, 119, 90, 255, 0, 255]
        self.orangeThreshold = [0, 18, 100, 255, 0, 216]
        self.yellowThreshold = [26, 30, 119, 255, 0, 255]
        self.greenThreshold = [57, 81, 83, 255, 0, 255]
        self.blackThreshold = [18, 78, 0, 87, 22, 112]
        self.squareThreshold = [15, 31, 74, 144, 0, 157]
        self.wallThreshold = [22, 42, 4, 72, 204, 255]
        self.wallThresholdInRow = [30, 60, 7, 50, 188, 255]
    ...
    def isCircle(self, contour):
        ...
        if (0.75 < circularity < 1.25):# Check for circle
            #is a circle
            return True
        else:
            return False
    ...
    def rowMarker(self, writeFrame, frameHSV, thresholdVals):
        ...
        cv2.imshow("markers", mask)
        if cv2.countNonZero(mask) > self.minArea:
            kernel = np.ones((2,2),np.uint8)
            # maskClose = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
            # maskCloseOpen = cv2.morphologyEx(maskClose, cv2.MORPH_OPEN, kernel)
            maskDilate = cv2.dilate(mask, kernel)
            cv2.imshow("markers", maskDilate)
            contours, heirarchy = cv2.findContours(maskDilate, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            filteredContours = []
            for contour in contours:
                area = cv2.contourArea(contour)
                if area > (self.minArea/8):
                    filteredContours.append(contour)
    ...
    def walls(self, writeFrame, frame, threshValsBlue, threshValsMiddle, threshValsRow):
        height, width = frame.shape[:2]
        leftThird = frame[:, :width // 3]
        rightThird = frame[:, width // 3:]
        leftThirdMask = self.threshold(leftThird, threshValsBlue)
        rightThirdMask = self.threshold(rightThird, threshValsBlue)
        if np.any(leftThirdMask > self.minArea) and np.any(rightThirdMask > self.minArea):
            thresholdVals = threshValsRow #shelf either side, use row threshold
        else:
            thresholdVals = threshValsMiddle
    ...
    def GetDetectedWallPoints(self):
        ...
        wallFrame, outputRB = self.walls(wallFrame, frameHSV, self.blueThreshold, self.wallThreshold, self.wallThresholdInRow)
